...
def get_bed(bed_id: int, db: Session = Depends(get_db)):
    ...
    restraint = (
        db.query(models.Restraint)
        .filter(models.Restraint.bed_id == bed_id)
        .order_by(models.Restraint.start_time.desc())
        .first()
    )
    ...
    return {
        ...
        "start_time": restraint.start_time if restraint else None
    }
